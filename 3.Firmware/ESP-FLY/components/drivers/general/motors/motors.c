...
void motorsInit(const MotorPerifDef **motorMapSelect)
{
    ...
    // 在配置LEDC之前，先复位GPIO并设置为低电平输出，避免上电时GPIO处于不确定状态导致电机转动
    for (i = 0; i < NBR_OF_MOTORS; i++)
    {
        gpio_reset_pin(motors_channel[i].gpio_num);
        gpio_set_direction(motors_channel[i].gpio_num, GPIO_MODE_OUTPUT);
        gpio_set_level(motors_channel[i].gpio_num, 0);
    }

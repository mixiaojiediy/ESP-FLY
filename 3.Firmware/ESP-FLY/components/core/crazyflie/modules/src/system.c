...
// #include "buzzer.h"  // Buzzer hardware removed
#include "sound.h"
#include "sysload.h"
...
void systemInit(void)
{
  ...
  workerInit();
  adcInit();
  // ledseqInit(); // LED disabled
  pmInit();
  // buzzerInit(); // Buzzer disabled
...
bool systemTest()
{
  ...
  // pass &= ledseqTest(); // LED disabled
  pass &= pmTest();
  DEBUG_PRINTI("pmTest = %d", pass);
  pass &= workerTest();
  DEBUG_PRINTI("workerTest = %d", pass);
  // pass &= buzzerTest(); // Buzzer disabled
  return pass;
}
...
void systemTask(void *arg)
{
  ...
  // pass &= configblockTest(); // EEPROM not available
  DEBUG_PRINTI("configblockTest = %d (skipped)", pass);
  // pass &= storageTest();
  pass &= commTest();
  ...
  if (pass)
  {
    ...
    // ledseqRun(&seq_alive); // LED disabled
    // ledseqRun(&seq_testPassed); // LED disabled
  }
  else
  {
    selftestPassed = 0;
    if (systemTest())
    {
      while (1)
      {
        // ledseqRun(&seq_testFailed); // LED disabled
        vTaskDelay(M2T(2000));
    ...
    {
      // ledInit(); // LED disabled
      // ledSet(SYS_LED, true); // LED disabled
    }
  }

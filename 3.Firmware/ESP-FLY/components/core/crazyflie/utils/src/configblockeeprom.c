...
// #include "eeprom.h"  // EEPROM hardware removed

// EEPROM hardware removed - provide empty stubs for compatibility
static inline void eepromInit(void* dev) { (void)dev; }
static inline bool eepromTestConnection(void) { return false; }
static inline bool eepromTest(void) { return false; }
static inline bool eepromReadBuffer(uint8_t* buffer, uint16_t addr, uint16_t len) { 
    (void)buffer; (void)addr; (void)len; 
    return false; 
}
static inline bool eepromWriteBuffer(const uint8_t* buffer, uint16_t addr, uint16_t len) { 
    (void)buffer; (void)addr; (void)len; 
    return false; 
}

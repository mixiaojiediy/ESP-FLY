...
#include "i2c_drv.h"
#include "mpu6050.h"
// Removed unused sensor headers: hmc5883l, ms5611, zranger, zranger2, vl53l1x, flowdeck

// Define constants for removed sensors (for compatibility)
...
static bool processGyroBias(int16_t gx, int16_t gy, int16_t gz, Axis3f *gyroBiasOut)
{
    ...
    if (!gyroBiasRunning.isBiasValueFound)
    {
        ...
        if (gyroBiasRunning.isBiasValueFound)
        {
            ...
            // ledseqRun(&seq_calibrated); // LED disabled
...
PARAM_ADD(PARAM_UINT8 | PARAM_RONLY, HMC5883L, &isMagnetometerPresent)
// PARAM_ADD(PARAM_UINT8 | PARAM_RONLY, pmw3901, &isPmw3901Present) // PMW3901 sensor removed
PARAM_ADD(PARAM_UINT8 | PARAM_RONLY, MS5611, &isBarometerPresent) // TODO: Rename MS5611 to LPS25H. Client needs to be updated at the same time.
PARAM_GROUP_STOP(imu_tests)

// 获取当前传感器数据的副本（直接访问，不通过队列）

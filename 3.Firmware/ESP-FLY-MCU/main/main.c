...
void app_main()
{
    ...
    */
    ...
    /* Initialize Status LED and set to booting state */
    if (statusLedInit()) {
        statusLedSet(STATUS_LED_BOOTING);  // Green blinking during boot
    }
    ...
    if (platformInit() == false) {
        statusLedSet(STATUS_LED_ERROR);  // Red LED on platform init failure
